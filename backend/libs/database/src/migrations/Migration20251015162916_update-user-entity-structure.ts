...
export class Migration20251015162916_update-user-entity-structure extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table "user_history" add constraint "user_history_user_user_id_foreign" foreign key ("user_user_id") references "user_master" ("user_id") on update cascade;`);

    this.addSql(`drop table if exists "users" cascade;`);
  }

  override async down(): Promise<void> {
    this.addSql(`alter table "user_history" drop constraint "user_history_user_user_id_foreign";`);
