...
const config = defineConfig({
  ...
  dbName: configService.get<string>('DB_DATABASE', 'collectors_card'),
  ...
  replicas: configService.get<string>('DB_READ_HOST') ? [
    {
      ...
      port: configService.get<number>('DB_READ_PORT', configService.get<number>('DB_PORT', 3306)),
      user: configService.get<string>('DB_READ_USERNAME', configService.get<string>('DB_USERNAME', 'root')),
      password: configService.get<string>('DB_READ_PASSWORD', configService.get<string>('DB_PASSWORD', '')),
      dbName: configService.get<string>('DB_DATABASE', 'collectors_card'),
    },
  ] : undefined,
});
